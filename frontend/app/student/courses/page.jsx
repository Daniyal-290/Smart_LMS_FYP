          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {courses.map((course) => (
              <div key={course._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 hover:shadow-md transition">
                ...
                <button
                  onClick={() => {
                    setNavigating(course._id);
                    router.push(`/student/course/${course._id}`);
                  }}
                  disabled={navigating === course._id}
                  className="w-full mt-7 py-3 rounded-xl bg-slate-800 text-white font-semibold hover:bg-slate-900 transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {navigating === course._id ? <Loader2 size={18} className="animate-spin" /> : null}
                  {navigating === course._id ? "Loading..." : "View Course"}
                </button>
              </div>
            ))}
          </div>
            ...
            {availableCourses.map((course) => (
              <div key={course._id} className="bg-white rounded-2xl border border-dashed border-slate-300 bg-slate-50 shadow-sm p-6">
                ...
                <button
                  ...
                  className="w-full mt-5 py-2 rounded-xl bg-amber-500 text-slate-900 font-semibold hover:bg-amber-600 transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {enrolling === course._id ? <Loader2 size={18} className="animate-spin" /> : null}
