                ...
                <div className="flex flex-col items-start lg:items-end justify-between gap-4">
                  ...
                  <div className="flex gap-3">
                    {assignment.status === "Pending" && (
                      <button
                        onClick={() => {
                          setNavigating(assignment._id);
                          router.push(`/student/submit-assignment?assignmentId=${assignment._id}`);
                        }}
                        disabled={navigating === assignment._id}
                        className="rounded-lg bg-slate-800 text-white px-5 py-2 hover:bg-slate-700 transition flex items-center justify-center gap-2 disabled:opacity-50"
                      >
                        {navigating === assignment._id ? <Loader2 size={16} className="animate-spin" /> : null}
                        {navigating === assignment._id ? "Loading..." : "Submit"}
                      </button>
                    )}
                  </div>
                </div>
