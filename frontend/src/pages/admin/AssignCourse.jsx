...
import { BookOpen, UserCheck, CalendarDays, Hash } from "lucide-react";
...
export default function AssignCourse() {
  ...
  const handleChange = (e) => {
    ...
    // Auto-fill Course ID when Course Name changes
    if (name === "name") {
      const selectedCourse = COURSE_CATALOG.find(c => c.name === value);
      setFormData((prev) => ({
        ...prev,
        name: value,
        id: selectedCourse ? selectedCourse.id : prev.id,
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/courses`, {
        ...
        body: JSON.stringify({
          ...
          description: `Schedule: ${formData.schedule} | Term: ${formData.term} | ID: ${formData.id}`,
      ...
      setFormData({ id: "", name: "", teacher: "", schedule: "", term: "Spring 2026", credits: 3 });
    } catch (err) {
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid md:grid-cols-2 gap-5">
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-300 px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-800 focus:border-transparent transition appearance-none"
              >
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-slate-100 border border-slate-300 px-4 py-3 rounded-xl focus:outline-none text-slate-600 transition"
                readOnly
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700 flex items-center gap-2">
              <UserCheck size={16} /> Assign To Teacher
            </label>
            <select
              name="teacher"
              required
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-white font-semibold py-4 rounded-xl transition-colors mt-4"
          >
