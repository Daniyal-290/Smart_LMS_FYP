  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-7xl mx-auto bg-white border border-slate-200 rounded-3xl shadow-sm p-8">
        ...
        >
          {navLoading ? <Loader2 size={18} className="animate-spin" /> : <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>}
                ...
                submissions.map(item => (
                  <tr key={item._id} className="border-b hover:bg-slate-50 transition">
                    ...
                    <td className="text-center">
                      ...
                      >
                        {gradingIds[item._id] ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
                        {gradingIds[item._id] ? "Grading..." : "Auto Grade"}
                      </button>
                    </td>
                  </tr>
                ))
